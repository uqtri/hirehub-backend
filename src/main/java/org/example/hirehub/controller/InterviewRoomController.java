...
public class InterviewRoomController {
    ...
    @PostMapping("/questions/{questionId}/mark-sent")
    public ResponseEntity<Map<String, Object>> markQuestionAsSent(
            @PathVariable Long questionId) {
        InterviewQuestionDTO question = interviewService.markQuestionAsSent(questionId);
        Map<String, Object> response = new HashMap<>();
        response.put("success", true);
        response.put("data", question);
        response.put("message", "Question marked as sent successfully");
        return ResponseEntity.ok(response);
    }
