...
public class TokenRefreshFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(accessToken != null && refreshToken != null && !jwtService.validateToken(accessToken)) {
            String email = jwtService.extractSubject(accessToken);

            if(redisService.isExistRefreshToken(email, refreshToken) && jwtService.validateToken(refreshToken)) {
