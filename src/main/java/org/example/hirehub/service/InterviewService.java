...
public class InterviewService {
    ...
    public InterviewRoomDTO createInterviewRoom(CreateInterviewRoomDTO dto) {
        ...
        // Create system message (before sending notifications to ensure it's in
        // transaction)
        createSystemMessage(savedRoom, "Interview room created. Waiting for participants to join.");

        // Send notification and email (after transaction commits, won't affect room
        // creation if fails)
        try {
    ...
    public boolean validateUserAccess(String roomCode, Long userId) {
        ...
        // Allow access to view expired/finished rooms (read-only mode will be handled
        // in frontend)
    ...
    public InterviewQuestionDTO answerQuestion(Long questionId, AnswerQuestionDTO dto) {
        ...
        if (pendingCount == 0) {
            ...
            // .content(question.getRoom().getApplicant().getName() + " has completed the
            // interview")
    ...
    private void sendResultEmail(InterviewRoom room, InterviewResult result) {
        try {
            ...
            // .content("Your interview result for " + room.getJob().getTitle() + " is
            // ready")
    ...
    @Transactional
    public InterviewQuestionDTO markQuestionAsSent(Long questionId) {
        InterviewQuestion question = interviewQuestionRepository.findById(questionId)
                .orElseThrow(() -> new ResourceNotFoundException("Question not found"));

        // Update status from PENDING to SENT
        question.setStatus("SENT");
        InterviewQuestion saved = interviewQuestionRepository.save(question);
